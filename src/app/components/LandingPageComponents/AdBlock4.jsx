import Image from 'next/image';

const AdBlock4 = () => {
  return (
    <div className="flex items-center bg-white p-4 drop-shadow-lg border md:rounded-md mt-10  md:w-[90%] mx-auto ">
      <div className="flex-shrink-0 relative hidden md:block">
        <Image src="/adblock4bg.png" alt="Ad 4" width={200} height={200} />
        <Image
          src="/adblock4trophy.png"
          alt="Ad 4"
          width={90}
          height={90}
          className="absolute top-1/2 -translate-y-1/2 left-[45%] -translate-x-1/2"
        />
      </div>
      <div className="ml-4 grow flex flex-col items-center gap-2">
        <Image
          src="/adblock4trophy.png"
          alt="Ad 4"
          width={90}
          height={90}
          className="md:hidden"
        />
        <h2 className="text-2xl md:text-3xl font-bold text-center">
          India&apos;s Most Convenient College Application Form
        </h2>
        <p className="text-gray-600 text-center">
          Apply to 2000+ colleges with just one form!
        </p>
        <button className="mt-2 bg-[#2914B5] text-white block py-2 px-6 rounded-md max-md:w-full">
          Start your Application
        </button>
      </div>
    </div>
  );
};

export default AdBlock4;
